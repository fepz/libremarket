...
defmodule Compras do
  ...
  def seleccionar_forma_entrega(compra, forma_de_entrega) do
    case forma_de_entrega do
      ...
      :correo -> 
        costo = :erpc.call(:"envios@hp", Envios.Server, :calcular_costo, [compra.id])
        %Compra{compra | envio: %{:metodo => :correo, :costo => costo}}
    end
  end
  ...
  defp reservar_producto(compra) do
    ...
    reserva = :erpc.call(:"ventas@hp", Ventas.Server, :reservar_producto, [compra.id])
    compra = %Compra{compra | producto: %{compra.producto | reservado: reserva}}
    {reserva, compra}
  end

  defp detectar_infraccion(compra) do
    infraccion = :erpc.call(:"infracciones@hp", Infracciones.Server, :detectar_infraccion, [compra.id])
    compra = %Compra{compra | infraccion: infraccion}
    {infraccion, compra}
  end

  defp autorizar_pago(compra) do
    pago = :erpc.call(:"pagos@hp", Pagos.Server, :autorizar_pago, [compra.id])
  ...
  def confirmar_compra(compra) do
    ...
    do
      if compra.envio.metodo == :correo do
        :erpc.call(:"envios@hp", Envios.Server, :agendar_envio, [compra.id])
      end
